...
/**
    ...
    * mbleed, written 4/2009 for OER162
 	...
 	public function coobjects_by_type($cotype = '')
	{
		...
		$sql = "o.location, o.name as oname, o.citation, o.action_type, o.done,
				s.name as sname, s.type_id, 
				t.type,
				of.filename as ofilename,
				mf.filename as mfilename,
				cf.filename as cfilename,
				o.id as oid, m.id as mid, cf.course_id as cid
        FROM ocw_objects as o, 
        		ocw_object_subtypes as s,
        		ocw_object_types as t,
        		ocw_object_files as of,
        		ocw_material_files as mf,
        		ocw_course_files as cf,
        		ocw_materials as m
        WHERE o.subtype_id = s.id AND
        		s.type_id = t.id AND
        		o.id = of.object_id AND
        		o.material_id = mf.material_id AND
        		m.id = o.material_id AND
        		m.course_id = cf.course_id AND
        		o.subtype_id = $cotype
";
    ...
    * mbleed, written 4/2009 for OER162
     */
 	public function coobject_types()
	{
		$objects = array();
		
		$sql = "s.name, s.id FROM ocw_object_subtypes AS s ORDER BY name ASC";
		$this->db->select($sql, FALSE);
		$q = $this->db->get();
