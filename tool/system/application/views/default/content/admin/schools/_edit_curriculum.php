...
<?php if ($curr !== false && is_array($curr)) { ?>

	<h2>Edit <?=$curr['name']?>'s Info</h2>

<form name="adminform" method="post" action="<?php echo site_url("admin/curriculum/edit_curriculum/{$sid}/{$curr['id']}")?>" enctype="multipart/form-data" style="margin:0px;">

	<table>
		<tr>
			<th style="text-align:right"><span style="color:red">*</span> Name: &nbsp;&nbsp;</th>
			<td><input type="text" id="name" name="name" tabindex="1" size="40" value="<?=$curr['name']?>" /></td>
		</tr>

		<tr>
			<th style=""> Description: &nbsp;&nbsp;</th>
			<td><textarea id="description" name="description" tabindex="2" style="width:330px; height:100px;"> <?=$curr['description']?> </textarea></td>
		</tr>
		
		<tr>
			<th></th>
			<td><input id="submitbutton" type="submit" name="submit" value="Edit Curriculum" /></td>
		</tr>
 	</table>
