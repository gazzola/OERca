...
class OCW_utils {
	...
	function create_co_img($cid, $mid, $name, $loc, $linkable=true, $shrink=true, $show_ctx=true) 
	{
	   ...
	   return ($linkable) 
				? $aurl.'<img id="'.$name.'" class="carousel-image tooltip" '.$title.' src="'.$imgUrl.'" '. $size .'"/></a>'.$slide
				: '<img id="'.$name.'" class="carousel-image tooltip" '.$title.' src="'.$imgUrl.'" '.$size.' />'.$slide;
	}
	...
	function replacement_exists($name) 
	{
	   $imgpath = ereg_replace("\.",'/',$name);
	   $p_imgurl = property('app_uploads_url').$imgpath.'/'.$name.'_rep.png';
	   $p_imgpath = property('app_uploads_path').$imgpath.'/'.$name.'_rep.png';
