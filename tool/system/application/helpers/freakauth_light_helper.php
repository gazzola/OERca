 ...
 * $obj->config->item('FAL_template_dir').login_anchor_guest
 * and
 * $obj->config->item('FAL_template_dir').login_anchor_user
 * 
...
function loginAnchor($login_uri = null, $logout_uri = null, $logout_attributes = null, $login_attributes = null)
{
	...
	$tpl_dir = $obj->config->item('FAL_template_dir').'auth/';
	if (belongsToGroup())
	{
		// if we have a user
		...
		return $obj->load->view($tpl_dir.'login_anchor_user', $data, true);
	}
	else
	{
		// if we have a guest
		...
		return $obj->load->view($tpl_dir.'login_anchor_guest', $data, true);
	}
}
